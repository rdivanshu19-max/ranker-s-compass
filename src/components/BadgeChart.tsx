...
const BADGE_SYSTEM = [
  { type: 'consistent', name: 'Consistent Learner', emoji: '🔥', requirement: '7-day study streak', description: 'Study every day for 7 days in a row using the study timer', maxProgress: 7 },
...
function ProgressRing({ progress, earned, size = 60, strokeWidth = 4 }: { progress: number; earned: boolean; size?: number; strokeWidth?: number }) {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl bg-primary/5 border border-primary/20 p-3">
        <p className="text-xs text-muted-foreground">
          <strong className="text-foreground">How it works:</strong> Badges are awarded automatically when you reach milestones.
          The <span className="text-primary font-medium">🔥 Consistent Learner</span> badge requires using the <strong>Study Timer</strong> on your Dashboard for 7 consecutive days — each day you must save at least 1 minute of study time.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {BADGE_SYSTEM.map(badge => {
          ...
          return (
            <div key={badge.type}
              className={`relative flex items-center gap-4 rounded-xl px-4 py-3 border transition-all overflow-hidden ${
                earned
                  ? 'bg-primary/5 border-primary/30 shadow-[0_0_15px_-3px_hsl(var(--primary)/0.3)]'
                  : 'bg-muted/30 border-border'
              }`}>
              {/* Shine/glow animation for earned badges */}
              {earned && (
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                  <div className="absolute -inset-full animate-[shine_3s_ease-in-out_infinite] bg-gradient-to-r from-transparent via-primary/10 to-transparent skew-x-12" />
                </div>
              )}
              <ProgressRing progress={pct} earned={earned} />
              <div className="flex-1 min-w-0 relative z-10">
                <div className="flex items-center gap-2">
                  <span className={`text-xl ${earned ? 'animate-[pulse_2s_ease-in-out_infinite]' : ''}`}>{badge.emoji}</span>
                  <p className={`font-medium text-sm ${earned ? 'text-primary' : ''}`}>{badge.name}</p>
                  {earned && <span className="text-[9px] bg-primary/20 text-primary px-1.5 py-0.5 rounded-full font-bold uppercase">Earned</span>}
                </div>
