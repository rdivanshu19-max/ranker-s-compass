...
serve(async (req) => {
  ...
  try {
    ...
    if (!has("consistent")) {
      ...
      if (streak >= 7) {
        await adminClient.from("user_badges").insert({ user_id: uid, badge_type: "consistent", badge_name: "🔥 Consistent Learner" });
        awarded.push("Consistent Learner");
      }
    }
    ...
    if (!has("explorer")) {
      ...
      if ((count || 0) >= 15) {
        await adminClient.from("user_badges").insert({ user_id: uid, badge_type: "explorer", badge_name: "🔍 Explorer" });
    ...
    if (!has("helpful")) {
      ...
      if ((count || 0) >= 3) {
        await adminClient.from("user_badges").insert({ user_id: uid, badge_type: "helpful", badge_name: "🤝 Helpful" });
        awarded.push("Helpful");
      }
    }
    ...
    if (!has("topper")) {
      ...
      if (hasTop) {
        await adminClient.from("user_badges").insert({ user_id: uid, badge_type: "topper", badge_name: "🏆 Topper" });
        awarded.push("Topper");
      }
    }
    ...
    if (!has("veteran")) {
      ...
      if ((count || 0) >= 30) {
        await adminClient.from("user_badges").insert({ user_id: uid, badge_type: "veteran", badge_name: "⭐ Veteran" });
